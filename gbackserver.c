...
int main() {
    ...
    printf("Enter the index of the frame where the error is to be shown: ");
    scanf("%d", &error_index);

    array[0] = htonl(frame_no);
    array[1] = htonl(win_size);
    array[2] = htonl(error_index);
    ...
    for (i = 1; i <= win_size; i++) {
        ...
        printf("\nSending to the client frame no: %d", i);
    }
    ...
    while (index <= frame_no) {
        if (ack == error_index || flag == 0) {
            index = ack;
            printf("\nNo Acknowledgment received from the client: ");

            for (j = ack + 1; j <= ack + win_size; j++) {
                temp[0] = htonl(j);
                send(client_socket, temp, sizeof(temp), 0);
                printf("\nSending to the client frame no: %d", j);
                index++;
            }
        ...
        if (ack != error_index || flag == 1) {
            printf("\nReceived Acknowledgment for frame no: %d", ack + 1);
            printf("\nSending to the client frame no: %d", index + 1);
            temp[0] = htonl(index);
            send(client_socket, temp, sizeof(temp), 0);
            i++;
            ack++;
